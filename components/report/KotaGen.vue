                  ...
                  <table class="w-full text-sm table-auto no-border">
                    <tbody>
                      <tr class="">
                        ...
                        <td class="px-2 ">:</td>
                        <td class="">{{ rawData.metadata.kota[props.kota].jumlah_penduduk }}</td>
                      </tr>
                      <tr class="">
                        <td class="">2. Jumlah Angkatan Kerja Kab./Kota</td>
                        <td class="px-2 ">:</td>
                        <td class="">{{ rawData.metadata.kota[props.kota].jumlah_angkatan_kerja }}</td>
                      </tr>
                      <tr class="">
                        <td class="">3. Jumlah Tenaga Kerja Bekerja Kab./Kota</td>
                        <td class="px-2 ">:</td>
                        <td class="">{{ data2["TOTALS"]["Jumlah Tenaga Kerja (Bekerja) Agustus"].toLocaleString('id-ID') }}</td>
                      </tr>
                      <tr class="">
                        <td class="">4. Produk Domestik Regional Bruto (ADHK) Kab./Kota</td>
                        <td class="px-2 ">:</td>
                        <td class="">Rp {{ data2["TOTALS"]["PDRB (ADHK) (juta Rp)"].toLocaleString('id-ID') }}
                        </td>
                      </tr>
                      <tr class="">
                        <td class="">5. Upah Rata-Rata Tenaga Kerja Kab./Kota</td>
                        <td class="px-2 ">:</td>
                        <td class="">Rp {{ data2["TOTALS"]["Jumlah Rata-Rata Upah/Pekerja"].toLocaleString('id-ID') }}</td>
                      </tr>
                      <tr class="">
                        <td class="">6. Jumlah Lapangan Usaha Kab./Kota</td>
                        <td class="px-2 ">:</td>
                        <td class="">{{ rawData.metadata.kota[props.kota].jumlah_lapangan_usaha }}</td>
                      </tr>
                      <tr class="">
                        <td class="">7. Jumlah Jam Kerja Kab./Kota</td>
                        <td class="px-2 ">:</td>
                        <td class="">{{ data2["TOTALS"]["Jumlah Rata-Rata Jam Kerja/Pekerja"].toLocaleString('id-ID') }} jam</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div class="content page-3">
                ...
                <div class="border border-black p-2 mb-2">
                  ...
                  <p class="text-lg font-bold mt-2">Hasil Pengukuran Produktivitas Sektoral Per Lapangan Usaha (Dalam
                    Diagram)</p>
                  <p class="text-center text-sm">Tabel 1. Pengukuran Produktivitas Makro {{ props.kota === 'Kepulauan Seribu'? 'Kabupaten' : 'Kota' }} Administrasi {{ props.kota }}</p>
...
const props = defineProps(["kota"])

const data1 = rawData.data[props.kota]
const data2 = rawData.raw[props.kota]
...
const data_1 = computed(() => {
  ...
  let calcData = data1["TOTAL"]["produktivitas"].map(x => x ? parseFloat(x.replaceAll('.', '')) : x)
...
const data_2 = computed(() => {
  ...
  let calcData = data1["TOTAL"]["growth_produktivitas"].map(x => x ? parseFloat(x.replace(',', '.').replace('%', '')) : x)
  dataset.datasets[0].data = calcData
  return dataset
})
...
const data_3 = computed(() => {
  ...
  let calcData = data1["TOTAL"]["upah"].map(x => x ? parseFloat(x.replaceAll(',', '.')) : x)
  dataset.datasets[0].data = calcData
  return dataset
})
...
const data_4 = computed(() => {
  ...
  let calcData = data1["TOTAL"]["jam_kerja"].map(x => x ? parseFloat(x.replaceAll('.', '')) : x)
  dataset.datasets[0].data = calcData
  return dataset
})

const data_6 = computed(() => {
  const provinsiRaw = data2
  const provinsiAnalysis = data1
